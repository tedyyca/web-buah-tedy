<!DOCTYPE html>
<html lang="id">
<head>
  <meta charset="utf-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1" />
  <title>Tedy Fruit Store — Buah Segar</title>
  <meta name="description" content="Tedy Fruit Store - Toko buah segar lokal & impor. Produk berkualitas, pengiriman Samarinda." />
  <link rel="stylesheet" href="styles.css" />
</head>
<body>
  <?php
    // Cek apakah ada query string 'promo' di URL
    if (isset($_GET['promo']) && $_GET['promo'] === 'BUAHSEGAR25') {

      $promo_code = htmlspecialchars($_GET['promo']);
      echo "<div style='text-align:center; padding:10px; background-color:#2f855a; color:white;'>";
      echo "Selamat! Anda menggunakan kode promo: <strong>$promo_code</strong>. Dapatkan diskon spesial!";
      echo "</div>";
    }
  ?>

  <header class="site-header">
    <div class="container header-inner" role="banner">
      <div class="branding">
        <div>
          <h1 class="brand-title">Tedy Fruit Store</h1>
          <p class="brand-sub">Buah segar lokal & impor — kualitas untuk keluarga</p>
        </div>
      </div>

      <nav aria-label="Main navigation">
        <ul class="nav-list">
          <li><a href="#beranda">Beranda</a></li>
          <li><a href="#produk">Produk</a></li>
          <li><a href="#tentang">Tentang Kami</a></li>
          <li><a href="#kontak">Kontak</a></li>
          <li><a href="login.php">Login Admin</a></li>
        </ul>
      </nav>
    </div>
  </header>

  <main class="container" id="main">
    <section id="beranda" aria-labelledby="beranda-heading">
      <h2 id="beranda-heading">Selamat Datang di Tedy Fruit Store</h2>
      <p class="lead">Toko buah segar terpercaya dengan pilihan buah lokal dan impor terbaik. Pengiriman Samarinda setiap hari.</p>
    </section>

    <section id="produk" aria-labelledby="produk-heading">
      <div class="section-head">
        <h2 id="produk-heading">Daftar Buah</h2>
      </div>

      <div class="products" role="list">
        <article class="card product" role="listitem" aria-labelledby="apelh-title">
          <h3 id="apelh-title">Apel Hijau</h3>
          <img class="product-img" src="pict/apel-hijau.jpg" alt="Apel hijau" width="180" height="160">
          <div class="price">Rp 50.000 <span class="unit">/ kg</span></div>
          <div class="btn-group">
            <button class="btn" type="button" aria-label="Beli Apel Hijau">Beli</button>
            <button class="btn btn--ghost" type="button" aria-label="Detail Apel Hijau">Detail</button>
          </div>
        </article>

        <article class="card product" role="listitem" aria-labelledby="apelm-title">
          <h3 id="apelm-title">Apel Merah</h3>
          <img class="product-img" src="pict/apel-merah.jpg" alt="Apel merah" width="180" height="160">
          <div class="price">Rp 60.000 <span class="unit">/ kg</span></div>
          <div class="btn-group">
            <button class="btn" type="button" aria-label="Beli Apel Merah">Beli</button>
            <button class="btn btn--ghost" type="button" aria-label="Detail Apel Merah">Detail</button>
          </div>
        </article>

        <article class="card product" role="listitem" aria-labelledby="jeruk-title">
          <h3 id="jeruk-title">Jeruk</h3>
          <img class="product-img" src="pict/jeruk.jpg" alt="Jeruk" width="180" height="160">
          <div class="price">Rp 34.000 <span class="unit">/ kg</span></div>
          <div class="btn-group">
            <button class="btn" type="button" aria-label="Beli Jeruk">Beli</button>
            <button class="btn btn--ghost" type="button" aria-label="Detail Jeruk">Detail</button>
          </div>
        </article>

        <article class="card product" role="listitem" aria-labelledby="nanas-title">
          <h3 id="nanas-title">Nanas</h3>
          <img class="product-img" src="pict/nanas.jpg" alt="Nanas" width="180" height="100">
          <div class="price">Rp 29.000 <span class="unit">/ buah</span></div>
          <div class="btn-group">
            <button class="btn" type="button" aria-label="Beli Nanas">Beli</button>
            <button class="btn btn--ghost" type="button" aria-label="Detail Nanas">Detail</button>
          </div>
        </article>

        <article class="card product" role="listitem" aria-labelledby="pepaya-title">
          <h3 id="pepaya-title">Pepaya</h3>
          <img class="product-img" src="pict/pepaya.jpg" alt="Pepaya" width="180" height="160">
          <div class="price">Rp 20.000 <span class="unit">/ buah</span></div>
          <div class="btn-group">
            <button class="btn" type="button" aria-label="Beli Pepaya">Beli</button>
            <button class="btn btn--ghost" type="button" aria-label="Detail Pepaya">Detail</button>
          </div>
        </article>

        <article class="card product" role="listitem" aria-labelledby="pisang-title">
          <h3 id="pisang-title">Pisang</h3>
          <img class="product-img" src="pict/pisang.jpg" alt="Pisang" width="180" height="160">
          <div class="price">Rp 16.000 <span class="unit">/ sisir</span></div>
          <div class="btn-group">
            <button class="btn" type="button" aria-label="Beli Pisang">Beli</button>
            <button class="btn btn--ghost" type="button" aria-label="Detail Pisang">Detail</button>
          </div>
        </article>

        </div>
    </section>

    <section id="tentang" aria-labelledby="tentang-heading">
      <div class="section-card">
        <h2 id="tentang-heading">Tentang Kami</h2>
        <p class="lead">Tedy Fruit Store berdiri sejak 2025 dan menyediakan buah-buahan segar langsung dari petani lokal maupun impor berkualitas. Kami berkomitmen menghadirkan produk sehat dengan harga terbaik untuk keluarga Anda. Pengiriman lokal setiap hari kerja.</p>
      </div>
    </section>

    <section id="kontak" aria-labelledby="kontak-heading">
      <div class="section-card">
        <h2 id="kontak-heading">Kontak</h2>
        <div class="contact-list">
          <p>Email: <a href="mailto:tedyfruitstore@gmail.com">tedyfruitstore@gmail.com</a></p>
          <p>Telepon: <a href="tel:+6282215854701">+62 822-1585-4701</a></p>
          <p>Alamat: Jl. Juanda, Samarinda</p>
        </div>
        <form class="contact-form" action="#" method="post" aria-label="Form kontak">
          <div>
            <label for="name">Nama</label>
            <input id="name" name="name" type="text" placeholder="Nama lengkap" required />
          </div>
          <div>
            <label for="email">Email</label>
            <input id="email" name="email" type="email" placeholder="email@domain.com" required />
          </div>
          <div>
            <label for="message">Pesan</label>
            <textarea id="message" name="message" rows="4" placeholder="Tinggalkan pesan untuk kami..."></textarea>
          </div>
          <div class="contact-actions">
            <button class="btn" type="submit" aria-label="Kirim Pesan">Kirim</button>
            <button class="btn btn--ghost" type="reset" aria-label="Reset Form">Reset</button>
          </div>
        </form>
      </div>
    </section>
  </main>

  <footer role="contentinfo">
    <div class="container">
      <p>Referensi: <a href="https://www.sayurbox.com/category/fruits-1-cdd2074a" target="_blank" rel="noopener">Sayurbox</a></p>
      <p>
        <a href="https://www.instagram.com/tedyyca" target="_blank" rel="noopener">
          </a>
      </p>
      <p>&copy; 2025 Tedy Fruit Store.</p>
    </div>
  </footer>
  <script src="script.js"></script>
</body>
</html>